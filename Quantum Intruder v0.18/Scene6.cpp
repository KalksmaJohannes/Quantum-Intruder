...
Scene6::Scene6(): _world(b2Vec2(0.0f, 100.0f)), _player(_world, toPixels(4.5), toPixels(16)), _deathsCount(0){
...
void Scene6::createMap() {
    ...
    _ramps.emplace_back(_world, toPixels(8), toPixels(8), true, true);
    _ramps.emplace_back(_world, toPixels(9), toPixels(9), true, true);
    ...
    /// Crear elevadores
    _elevators.emplace_back(_world, toPixels(11), toPixels(9), 9.0f, 15.0f, 2.0f);


    /// Crear pinchos
    for (i=7; i<19; i++){
        _spikes.emplace_back(_world, toPixels(i), toPixels(16.25), true);
    }

    _spikes.emplace_back(_world, toPixels(7), toPixels(3.76), false);
    ...
    /// Crear lasers
    _laserOriginalPosition = b2Vec2(22.5f, 6.5f);

    _lasers.emplace_back(_world, toPixels(0), toPixels(0), 5.0f);

    /// Crear llave
    _keyOriginalPosition = b2Vec2(27.0f, 8.2f);
    ...
    _levelImage.setTexture("resources/Sprites/bloqueNivel6.png");

}

void Scene6::draw(sf::RenderTarget& target, sf::RenderStates states) const{
...
b2World& Scene6::getWorld(){

    return _world;

}

void Scene6::resetScene(){
